...
# Independent top-level subsystem. Reuses the existing FastAPI
# app and (optionally) the QuantumLab experiment service. All
# endpoints are air-gap-safe; no external dependencies.
# ---------------------------------------------------------------------------

class SatsaIngestRequest(BaseModel):
    """SAT-SA CSE submission ingestion. Accepts either `json_payload`
    (a complete JSON submission object) or `csv_*` (per-entity CSV
    strings). Exactly one of json_payload / csv_alerts / csv_assets
    must be supplied."""
    json_payload: dict | None = None
    csv_assets: str | None = None
    csv_alerts: str | None = None
    csv_investigations: str | None = None
    csv_escalations: str | None = None
    csv_closures: str | None = None
    meta: dict | None = None
    processing_version: str = "1.0.0"


class SatsaDemoRequest(BaseModel):
    cse_id: str = "CSE-002"


@app.post("/api/satsa/ingest")
def satsa_ingest(req: SatsaIngestRequest):
    """Ingest a CSE submission and return the canonical normalized
    representation. Raises 400 on validation failure."""
    from ..satsa.ingestion import from_json, from_csv, IngestionError
    try:
        if req.json_payload is not None:
            sub = from_json(req.json_payload,
                              processing_version=req.processing_version,
                              sign=False)
        elif req.csv_alerts is not None or req.csv_assets is not None:
            if req.meta is None:
                raise http_error(400, "VALIDATION_ERROR",
                                   "CSV ingestion requires a 'meta' dict.")
            sub = from_csv(
                assets_csv=req.csv_assets, alerts_csv=req.csv_alerts,
                investigations_csv=req.csv_investigations,
                escalations_csv=req.csv_escalations,
                closures_csv=req.csv_closures,
                meta=req.meta,
                processing_version=req.processing_version,
            )
        else:
            raise http_error(400, "VALIDATION_ERROR",
                               "Provide either json_payload or at least "
                               "one csv_* field.")
    except IngestionError as e:
        raise http_error(400, "INGESTION_ERROR", str(e))
    return sub.to_dict()


@app.post("/api/satsa/demo")
def satsa_demo(req: SatsaDemoRequest):
    """Load a built-in demo assessment (CSE-002 execution gap by
    default). The demo is deterministic and includes ground truth."""
    from ..satsa.demo import load_demo_assessment
    return load_demo_assessment(req.cse_id)


@app.post("/api/satsa/run")
def satsa_run(req: SatsaIngestRequest):
    """Ingest the submission AND execute the full analytical pipeline
    (execution gap + negative space + anomaly workers). Returns the
    AnalysisRun with observations, decomposable risk, and a Merkle
    audit-root integrity hash."""
    from ..satsa.ingestion import from_json, from_csv, IngestionError
    from ..satsa.analysis_run import execute_run
    from ..satsa.workers.execution_gap import register_workers as _eg
    from ..satsa.workers.negative_space import register_workers as _ns
    from ..satsa.workers.anomaly import register_workers as _an
    from ..satsa.risk import derive_risk
    try:
        if req.json_payload is not None:
            sub = from_json(req.json_payload,
                              processing_version=req.processing_version,
            ...
            raise http_error(400, "VALIDATION_ERROR",
                               "Provide either json_payload or csv_*.")
    except IngestionError as e:
        raise http_error(400, "INGESTION_ERROR", str(e))
    _eg(); _ns(); _an()
    run = execute_run(sub)
...
@app.get("/api/satsa/workers")
def satsa_list_workers():
    """List registered analytical workers."""
    from ..satsa.analysis_run import list_workers
    return {"workers": list_workers()}


@app.get("/api/satsa/trust/verify")
def satsa_trust_verify(message_digest: str, signature: str,
                        public: str):
    """Verify a Lamport signature record. All three arguments are
    JSON-encoded lists of hex strings. Returns {verified: bool}."""
    from ..satsa.trust import verify_signature
    import json as _json
    try:
        sig = _json.loads(signature)
        pub = _json.loads(public)
    except Exception as e:
        raise http_error(400, "VALIDATION_ERROR", f"JSON parse: {e}")
    rec = {"message_digest": message_digest, "signature": sig,
           "public": pub}
    return {"verified": verify_signature(rec)}
