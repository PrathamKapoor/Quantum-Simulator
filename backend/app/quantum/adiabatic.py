...
and the adiabatic experiment: prepare the H_initial ground state,
evolve under H(s) with a time-dependent Schrodinger solver, and
measure the final ground-state overlap, excitation probability, and
...
def interpolated_hamiltonian(h_initial: np.ndarray, h_final: np.ndarray,
                             s: float) -> np.ndarray:
    """H(s) = (1 - s) H_initial + s H_final (Hermitian validated)."""
    if not (0.0 <= s <= 1.0):
...
def spectral_gap_curve(h_initial: np.ndarray, h_final: np.ndarray,
    ...
    for s in ss:
        evals, evecs = np.linalg.eigh(
            interpolated_hamiltonian(h_initial, h_final, s))
...
def adiabatic_evolution(h_initial: np.ndarray, h_final: np.ndarray,
    ...
    ground state. Returns the final state, its overlap with the H_final
    ground state, and (optionally) sampled measurement outcomes."""
    if total_time <= 0:
        raise QuantumCoreError("total_time must be positive.")
    if steps < 1:
        raise QuantumCoreError("steps must be >= 1.")
    evals0, evecs0 = np.linalg.eigh(
        interpolated_hamiltonian(h_initial, h_final, 0.0))
    state = evecs0[:, 0].astype(np.complex128)
    dt = total_time / steps
    for k in range(steps):
        s = (k + 0.5) / steps           # midpoint rule
        h = interpolated_hamiltonian(h_initial, h_final, s)
        state = expm(-1j * h * dt) @ state
    evalsf, evecsf = np.linalg.eigh(
        interpolated_hamiltonian(h_initial, h_final, 1.0))
    ground = evecsf[:, 0]
    ...
    result = {
        ...
        "min_gap": spectral_gap_curve(h_initial, h_final, 51)["min_gap"],
...
def adiabatic_scan(h_initial: np.ndarray, h_final: np.ndarray,
                   times: list[float], steps: int = 200) -> list[dict]:
